...
public class UserServiceTest {
	...
	private User getDummyUser() {
		String today = "2018-11-09 10:30";
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
		LocalDateTime ldt = LocalDateTime.parse(today, dtf);
		...
		user.setCreatedBy(1);
		user.setCreatedTime(ldt);
		user.setLastModifiedBy(1);
		user.setLastModifiedTime(ldt);
		return user;
	}
	...
	public void testFindByIdIn() {
		// case 1 : should be found the data
		List<Long> userIds = new ArrayList<Long>(Arrays.asList(1L, 2L, 3L));
		...
		List<Long> userIds2 = new ArrayList<Long>(Arrays.asList(1L));
	...
	public void testIsUserAvailable() {
		...
		Assert.assertTrue(userServiceMock.isUserAvailable("username", "cakpep").getAvailable());
		Assert.assertTrue(userServiceMock.isUserAvailable("email", "java@bootcamp.com").getAvailable());

		// case 2 : should be false
		Assert.assertFalse(userServiceMock.isUserAvailable("xxx", "java@bootcamp.com").getAvailable());
		Assert.assertFalse(userServiceMock.isUserAvailable("email", "notfound@bootcamp.com").getAvailable());
	}
